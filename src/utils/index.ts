...
export class Utils {
  ...
  static async astProcessFile(
    ...
    try {
      ...
      // Quick exit: if file contains no Chinese characters, skip heavy parsing.
      // This avoids expensive AST parsing for unrelated files.
      if (!Utils._containsChinese(code)) return null;
      ...
      (q) => async () => {
        return new Promise<{ q: string[]; data: any }>(
          async (resolve, reject) => {
            const params = {
              inputLanguage: langMap[defaultLang],
              query: q.join(SPLIT),
              cookie,
            };
